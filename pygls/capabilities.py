...
class ServerCapabilitiesBuilder:
    ...
    def _with_signature_help(self):
        value = self._provider_options(TEXT_DOCUMENT_SIGNATURE_HELP, default=SignatureHelpOptions())
        if value is not None:
    ...
    def _with_type_definition(self):
        value = self._provider_options(TEXT_DOCUMENT_TYPE_DEFINITION, default=TypeDefinitionOptions())
        if value is not None:
            self.server_cap.type_definition_provider = value
        return self

    def _with_implementation(self):
        value = self._provider_options(TEXT_DOCUMENT_IMPLEMENTATION, default=ImplementationOptions())
        if value is not None:
